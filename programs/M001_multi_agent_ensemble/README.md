...
| — | [`sim/`](sim/) | **Φ2.5 scaffold (2026-06-24).** Deterministic replay simulator + Streamlit v0 dashboard + regime classifier. Sub-tree: `sim/core/` (kernel), `sim/regime/` (4-class classifier + G4 gate), `sim/scoring/` (F12 TQS · F17 ΔInfo · F18 regime KPIs), `sim/roster/` (`mvp_phi4.yaml` + `full_canon.yaml`), `sim/agents/` (A1/A6/A7/A10 stubs + placeholder), `sim/dashboard/` (Streamlit, six panels), `sim/tests/` (43 tests, all passing). See [`sim/README.md`](sim/README.md). |

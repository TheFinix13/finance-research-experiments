...
| 06 | [`06-blue-lock-doctrine.md`](06-blue-lock-doctrine.md) | **The philosophical spine.** Translates ego, weapon, metavision, coordinate, chemical reaction, devour, awakening, TQS, and the human-as-opponent into typed objects, formulas, and measurable KPIs. **v0.3 adds §3.11 Agent Evolution Arcs** — every striker is a versioned identity (vN → vN+1) whose evolution is earned by a documented defeat / phase / inspiration trigger, never asserted. The §3.11.2 contract (regression test + forward test + co-existing roster registration) binds every vN+1 claim. Read this *before* `05` to understand what the cast is instantiating. |
| 07 | [`07-research-standards.md`](07-research-standards.md) | Φ2.5 standards: nested walk-forward CV, reproducibility manifests, experiment tracking, null-baseline suite, shadow-mode-first policy, locked human-adversary submission protocol. **v0.2 §10.1–§10.5** appended post-audit amendments (E010 parallel, agent promotion internal to M001, E008 skipped, hybrid verdict registry, v0.1 tier definitions superseded); **v0.3 §10.6** binds the doctrine §3.11 evolution-arcs contract to standards-grade evaluation (regression + forward tests mandatory). |
...
| — | [`sim/`](sim/) | **Φ4 v1 (2026-06-24).** Deterministic replay simulator + Streamlit v0 dashboard + regime classifier + 4-agent squad gate harness. Sub-tree: `sim/core/` (kernel + two-phase tick order), `sim/regime/` (4-class classifier + G4 gate), `sim/scoring/` (F12 TQS · F17 ΔInfo · F18 regime KPIs · Φ3 + Φ4 gate harnesses), `sim/roster/` (`mvp_phi4.yaml` + `full_canon.yaml`), `sim/agents/` (A1 Isagi v1 + A6 Nagi v1 + A7 Barou v1 + A10 Kunigami v1, all wrapping production cells or implementing doctrine predicates), `sim/dashboard/` (Streamlit, six panels), `sim/tests/` (125 tests + 3 slow skips, all passing). See [`sim/README.md`](sim/README.md). |
| — | [`reviews/`](reviews/) | **Φ3+ lab evidence ledger.** Per-gate markdown reviews + per-trade JSONLs. Current: [`reviews/phi3_gate_isagi_v1.md`](reviews/phi3_gate_isagi_v1.md) (Φ3→Φ4 wrapper validation, **PASS** @ +11.04 median OOS pips/trade); [`reviews/phi4_squad_v1.md`](reviews/phi4_squad_v1.md) (Φ4→Φ5 squad gate, **FAIL** @ 0.98× Isagi-alone TQS; honest diagnosis explains Nagi 0-trade starvation + Barou median dilution + concurrency rejections); [`reviews/phi4_isagi_rejection_analysis.md`](reviews/phi4_isagi_rejection_analysis.md) (cross-striker rejection-bucket analysis of 2994 Isagi rejections). [`reviews/evolution_ledger.md`](reviews/evolution_ledger.md) is the audit trail for every actual `vN → vN+1` event per doctrine §3.11.4 (no rows yet). Every replay run that claims a gate verdict lands here. |

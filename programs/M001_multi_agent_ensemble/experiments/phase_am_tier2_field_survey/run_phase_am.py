"""Phase AM Tier-2 first-wave survey: full squad on XAUUSD/XAGUSD/
USOIL/USTEC, design region 2015-01-01 -> 2022-12-31, equity=500.

    python run_phase_am.py --out-dir results/raw/survey \
...
SURVEY_SYMBOLS = ("XAUUSD", "XAGUSD", "USOIL", "USTEC")
DESIGN_START = "2015-01-01"
DESIGN_END = "2022-12-31"  # 2023+ sealed per DATA_LEDGER rule 4
EQUITY = 500.0  # the real v2 demo account (declared in PROTOCOL.md)
...
def main() -> None:
    ...
    for sym in SURVEY_SYMBOLS:
        df = pd.read_parquet(TIER2_STORE / f"{sym}_H4.parquet")

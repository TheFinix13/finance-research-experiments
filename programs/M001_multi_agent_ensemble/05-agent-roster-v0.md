...
| A9 | **Aoshi Tokimitsu** | Berserker | Overwhelming physicality (event mode) | Macro-event-only vol-breakout (FOMC / NFP / CPI) | `aoshi_tokimitsu` | M15 event-window (Phase AE cadence) | M15 follow-through, H1 fade-protection | 0.75 | Φ4+ | **implemented — Tier-0 v1-readiness only (2026-08-19), alpha PENDING Tier-1 pre-registration.** `sim/agents/a09_aoshi_v2.py` (`A9AoshiV2`) subclasses the Phase AE striker so trigger + mechanic geometry are inherited unchanged, and adds the missing v1 primitives: F19 `lot_intent`, F20 `risk_intent` (`event_specialist` now registered in both dispatch tables), F21 `read_workspace` + defensive abstention, geometry-driven conviction/regime-fit replacing v1's literal 0.85/0.60, a hard time stop at `target_hold_hours`, and nearest-release event selection. **Historical naming:** the Phase AE striker shipped under the id `sae_itoshi` (`sim/agents/a09_sae.py`, untouched per §10.6); sealed AE artifacts retain that id and are not rewritten. G7 C3–C6 now passable; **C1 (quality) and C2 (lifts a peer) remain open** — Phase AE's `FAIL` stands unrevised and Tier 0 is **not** an alpha claim. See `reviews/sae_itoshi_v1_defeat.md` §2 Tier 0. |
...
| **A9 Aoshi** | `pre-v1 (Tier-0 primitives landed 2026-08-19)` — F19/F20/F21 present in `sim/agents/a09_aoshi_v2.py`, so C3–C6 are passable; **C1 fails on the Phase AE alpha defeat (AE2: mean TQS 0.097 vs a 0.30 floor) and C2 has no measured lift.** Tier-0 plumbing is explicitly not an alpha claim | Event-specialist | Tier-1 trigger redesign (volatility-normalised, fade-only) — pre-registered separately |
...
| **Status** | **implemented, Tier-0 v1-readiness only (2026-08-19).** Module `sim/agents/a09_aoshi_v2.py`, class `A9AoshiV2`, alias `AoshiTokimitsu`, playstyle `event_specialist`, tier 1, home TF M15. Tests: `sim/tests/test_a09_aoshi_v2_regression.py`, `sim/tests/test_a09_aoshi_v2_resolves_ae2.py`. **Alpha is PENDING Tier-1 pre-registration; Tier 0 must never be reported as a performance improvement.** |
| **MVP Φ3?** | Φ4+ |
| **Canon role (fixed)** | Berserker; overwhelming physicality only in event mode (FOMC / NFP / CPI / ECB) |
| **Information tier status** | Tier 1 as constructed (matches the Phase AE striker's registration); F17 ΔInfo not yet measured |
| **Naming history (2026-08-19 operator decision)** | The A9 event striker was **built and evaluated under the id `sae_itoshi`** (Phase AE, `sim/agents/a09_sae.py`, class `A9SaeV1`). That collided with **"Sae Itoshi (foil)" / `opponent_sae`**, which is this program's frozen adversarial OPPONENT and heritage floor (§5.3 below; `07-research-standards.md` §4.2 "Frozen-Sae"). The striker is therefore renamed to its rightful roster slot, `aoshi_tokimitsu`, from `a09_aoshi_v2.py` onward. **Sealed Phase AE artifacts (`experiments/phase_ae_sae_event_specialist/**`, `reviews/phase_ae_verdict.md`) legitimately continue to say `sae_itoshi` / `sae_fade` / `sae_ride` and are byte-untouched**, as is `a09_sae.py` per `07-research-standards.md` §10.6. The opponent row keeps the Sae name. |
| **v1 checkpoint state (G7)** | **C3–C6 passable, C1 + C2 open.** Before 2026-08-19 the agent scored 1 of 6: `event_specialist` was absent from `sim/core/lot_intent.py` and `sim/core/risk_intent.py`, so F19/F20 silently took the fallbacks their module headers label "not a valid v1 implementation" (C5, C6); `intend(workspace=…)` was accepted and discarded (C4 read side); and `_build_proposal` emitted a literal `conviction=0.85` / `regime_fit=0.6` on every proposal, pinning dispersion at zero. Tier 0 closes those. **C1 still fails on Phase AE's AE2 (mean TQS 0.097, CI95 [0.042, 0.162] vs a 0.30 floor) and C2 has no measured lift** — the F21 abstention is the mechanism, not yet the evidence. |
| **Current version** | **v2 (Tier-0 v1-readiness), 2026-08-19.** v1 (`a09_sae.py`) remains on disk and untouched per the §10.6 retention rule. Phase AE's `FAIL` verdict is unrevised by this landing. |
| **Evolution arc** | Registered in `reviews/sae_itoshi_v1_defeat.md` (2026-08-19), three separable tiers. **Tier 0 — LANDED:** v1-readiness plumbing (F19/F20/F21, conviction function, hard time stop, nearest-release event selection). No panel consumed, no threshold touched, no alpha claimed. **Tier 1 — NOT IMPLEMENTED:** the alpha hypothesis — volatility-normalised `move ≥ m × ATR96` trigger with a pre-declared `m` from Phase AG's shape, and deletion of the `sae_ride` mechanic (61 trades, −5.88 pips/trade, 78 % of the book). Pre-registered separately; the honest prior in §6 of the defeat note is `parked_insufficient_n`. **Tier 2 — BLOCKED:** the Phase AI surprise panel, pending one MQL5 calendar export on the trading VM. Closed lanes (do not reopen): FOMC statement tone (Phase AH, `dead`), and softening v1's thresholds on the spent AE panel (pre-banned by AE PROTOCOL §5.1). |
| **Defeat trigger** | **Fired 2026-08-19** — Phase AE `FAIL`: AE2 quality floor missed decisively (mean TQS **0.097**, CI95 **[0.042, 0.162]** against ≥ 0.30 / CI-lower > 0.20), 28.7 % win rate at a fixed 1.5R target on 87 trades, and a 45× asymmetry between the two mechanics (`sae_fade` −0.13 pips/trade over 26 vs `sae_ride` −5.88 over 61). Separable second defeat: not a v1 *player* at all — 1 of 6 G7 bits, three failures being absent implementations, plus no time stop against observed holds of 6,675 minutes (111 h). Full citation chain: `reviews/sae_itoshi_v1_defeat.md` §1. |

...
| 2026-08-19 | Post-Phase-AE evolution arc, **Tier 0 only** (v1-readiness plumbing; no panel consumed) | A9 **Aoshi Tokimitsu** (`aoshi_tokimitsu`) — **RENAMED from `sae_itoshi`** | v1 (`a09_sae.py`, untouched) → v2 Tier-0 (`a09_aoshi_v2.py`) | **(i) Rename decision (operator, 2026-08-19).** The Phase AE event striker shipped as `sae_itoshi`, but `05-agent-roster-v0.md` assigns the A9 slot to **Aoshi Tokimitsu** ("Macro-event-only vol-breakout (FOMC / NFP / CPI)" — verbatim what this agent does), while **"Sae Itoshi (foil)" / `opponent_sae` is the frozen adversarial OPPONENT** and the heritage floor ("Frozen-Sae", `07-research-standards.md` §4.2). Two "Sae"s makes every future report ambiguous between a player and a benchmark, so the striker takes its rightful roster name and the opponent keeps Sae. **Sealed Phase AE artifacts (`experiments/phase_ae_sae_event_specialist/**`, `reviews/phase_ae_verdict.md`) and `sim/agents/a09_sae.py` are byte-untouched and legitimately still say `sae_itoshi` / `sae_fade` / `sae_ride`.** **(ii) Defeat trigger:** Phase AE `FAIL` — AE2 mean TQS **0.097**, CI95 **[0.042, 0.162]** vs floors 0.30 / 0.20; 28.7 % win at fixed 1.5R over 87 trades; 45× mechanic asymmetry (`sae_fade` −0.13 pips over 26 vs `sae_ride` −5.88 over 61). Separable second defeat: **1 of 6 G7 bits**, three failures being *absent implementations*, plus no time stop against observed 6,675-minute (111 h) holds. Note `reviews/sae_itoshi_v1_defeat.md`. | **Tier 0 ONLY, and Tier 0 is NOT an alpha claim.** Registered `event_specialist` in the F19 `sim/core/lot_intent.py` and F20 `sim/core/risk_intent.py` dispatch tables (pure addition — every existing playstyle's output pinned unchanged by golden-value test); implemented event-specialist F19 (`event_impulse_lot_intent`: violent print ⇒ wider stop ⇒ SMALLER lot) and F20 (`event_impulse_risk_intent`: ATR-driven stop, ladder whose **first rung is still v1's 1.5R**, so the inherited bracket is preserved); F21 `read_workspace` + abstention when a Tier-1 incumbent holds the slot in the opposite direction; conviction and regime-fit functions of `(move/ATR, wick_frac, minutes_since_release)` calibrated to return v1's literal 0.85 / 0.60 at v1's own trigger boundary (ratio 4.0, wick 0.50, T+15), replacing the hard-coded constants that pinned C5/C6 dispersion at zero; hard time stop at `target_hold_hours` (6.0); nearest-release event selection replacing v1's earliest-candidate quirk. `A9AoshiV2` **subclasses** `A9SaeV1` and `intend` delegates to `super().intend`, so direction / entry / stop are byte-identical by construction. **Trigger thresholds, fade/ride mechanics and the primary 1.5R target are untouched. No walk-forward panel, replay or scoring harness was run.** | Both on disk: v1 `a09_sae.py` remains canonical for gate-evaluation purposes and is the Phase AE artifact of record; v2 carries the roster name. Retirement of v1 is not proposed and is gated on a Tier-1 verdict, per `07-research-standards.md` §3 retention. | **TIER-0 LANDED — NOT AN ALPHA RESULT. Phase AE's `FAIL` stands unrevised and the arc is NOT closed.** G7 bits made **passable: C3, C4, C5, C6** (measured directly: `lot_intent` CV **0.356**, `risk_intent` SL / TP1 CV **0.329**, both against a 0.10 floor, over 108 plausible event geometries). Bits still **OPEN: C1** (quality — the AE2 alpha defeat, owned by Tier 1) and **C2** (lifts a peer — the F21 abstention is the mechanism, not yet the evidence). Tests: `sim/tests/test_a09_aoshi_v2_regression.py` (55) + `sim/tests/test_a09_aoshi_v2_resolves_ae2.py` (9) green, `sim/tests/test_a09_sae_sim.py` (12) unregressed, full sim suite 961 passed / 4 skipped. Tier 1 (volatility-normalised fade-only trigger) and Tier 2 (Phase AI surprise panel) are pre-registered separately and NOT implemented. |

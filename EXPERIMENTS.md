...
| Phase AD.2 Karasu window anchor semantics | 2026-07-28 (PROTOCOL + §1b pre-run amendment: Phase AE frozen USD fixture substitutes the never-created full-FF Phase AD fixture; phi41 physical panel = the R7-naive ledger; sim midnight H4 grid, labels = bar OPENS, entry = open+4h) | **NULL (2026-07-28, Stage 1 evaluated once, Stage 2 gate CLOSED)** — across **51,042** evaluation points (H4 open labels × EURUSD/GBPUSD/USDCAD, 2015-02..2025-12) × 349 frozen events, the ±15-min ladder fired **ZERO** times under BOTH the bar-open anchor (A, current live behaviour) and the entry-moment anchor (B): S1 = 0.0000 %, S2 = 0/5,236 admitted trades flipped. Anchor question moot; trading-agent live path keeps semantics A (I020 resolved, D131). Stronger finding: the point-anchored window is **structurally inert on the H4 grid** for NFP/CPI/FOMC (13:30/18:00/19:00 UTC never within 15 min of a 00/04/…/20 anchor). Audit color: C-style holding-window semantics is the only variant that engages (1,035 points; 166 trades ≈ 3.2 %) but the sampled C-gated trades were net WINNERS (+3.18 pips mean) — banked as the prior AGAINST a future C lever (own pre-reg required). Scope: USD-subset fixture + midnight grid; live full-calendar operation NOT claimed inert (2026-07-24 live "PMI in +15 min" advisory proves A fires there). | `programs/M001_multi_agent_ensemble/experiments/phase_ad2_karasu_window_semantics/{PROTOCOL.md,stage1_audit.py,results_stage1.json}`; `reviews/phase_ad2_verdict.md` |
